...
module.exports = function config(diceConstant, $routeProvider, $locationProvider, $sceDelegateProvider) {

    $routeProvider.when('/pay-per-view', {
        templateUrl: diceConstant.domains.customerAssetsProxy + '/src/admin/pay-per-view/pay-per-view.template.html',
        controller: 'PayPerViewController',
        controllerAs: 'vm'
    }).when('/pay-per-view/view-counts', {
        templateUrl: diceConstant.domains.customerAssetsProxy + '/src/admin/pay-per-view/view-counts.template.html',
        controller: 'ViewCountsController',
        controllerAs: 'vm'
    }).otherwise({
        redirectTo: '/pay-per-view'
    });
...
module.exports = function PayPerViewController($location, diceConstant, PayPerViewService) {
    ...
    vm.emails = [];
    vm.notify = [];
    vm.percentage = ['100', '95', '90', '75', '50', '25'];

    vm.addEmail = function (e) {
        e.target.blur();
        vm.emails.push(vm.newEmail);
        vm.newEmail = '';
    };

    vm.removeEmail = function (index, e) {
        e.target.blur();
        vm.emails.splice(index, 1);
    };

    vm.toggleCheck = function (key) {
        var index = vm.notify.indexOf(key);
        if (index > -1) {
    ...
    function resetForm() {
        vm.viewHardLimit = null;
        vm.viewTempLimit = null;
        vm.jobHardLimit = null;
        vm.jobTempLimit = null;
        vm.totalViews = null;
        vm.emails = [];
    };
    ...
    function buildPayload() {
        var payload = {};
        payload.viewLimit = vm.viewHardLimit;
        payload.tempViewLimit = vm.viewTempLimit;
        payload.jobLimit = vm.jobHardLimit;
        payload.tempJobLimit = vm.jobTempLimit;
        payload.notifyList = vm.emails;
        payload.notifyPercentages = vm.notify;
        payload.costPerClick = vm.costPerClick;
        return payload;
    };

    function getCounts() {
        if (getStatus === 200) {
            PayPerViewService.getCounts(companyId).then(function (response) {
                var data = response.data;
                vm.totalViews = data.totalTermViews;
            });
        }
    };

    function getCompanyName() {
        PayPerViewService.getCompanyName(companyId).then(function (response) {
            var data = response.data;
    ...
    vm.delete = function (e) {
        e.target.blur();
        vm.deleting = true;
        PayPerViewService.delete(companyId).then(function (response) {
            resetForm();
            vm.deleting = false;
            $location.path('/pay-per-view/view-counts');
        });
    };
    ...
    vm.save = function (e) {
        ...
        if (getStatus !== 200) {
            vm.saving = true;
            PayPerViewService.post(companyId, buildPayload()).then(function (response) {
                vm.saving = false;
                $location.path('/pay-per-view/view-counts');
            }, function () {
                vm.saving = false;
        ...
        if (getStatus === 200) {
            vm.saving = true;
            PayPerViewService.put(companyId, buildPayload()).then(function (response) {
                vm.saving = false;
                $location.path('/pay-per-view/view-counts');
            }, function () {
                vm.saving = false;
    ...
    PayPerViewService.get(companyId).then(function (response) {
        var data = response.data;
        vm.viewHardLimit = data.viewLimit;
        vm.viewTempLimit = data.tempViewLimit;
        vm.jobHardLimit = data.jobLimit;
        vm.jobTempLimit = data.tempJobLimit;
        vm.emails = data.notifyList;
        vm.costPerClick = data.costPerClick;
        vm.notify = data.notifyPercentages;
        ...
        if (response.status !== 404) {
            vm.errorMessage = $.grep(data.message.messages, function (e) {
                return e.userMessage;
            })[0].userMessage;
        } else {
            vm.errorMessage = null;
        }
        vm.notify = vm.percentage;
    });
};
...
require('../../common/1.3/dice/dice.module');

require('../../common/1.3/directive/float-label.directive');

require('../../common/1.3/filter/percent-filter/percent.filter');

angular.module('app', ['dice', 'ngRoute', 'dice.directive.floatLabel', 'dice.filter.percent']).config(require('./pay-per-view.config.js')).controller('PayPerViewController', require('./pay-per-view.controller.js')).controller('ViewCountsController', require('./view-counts.controller.js')).service('PayPerViewService', require('./pay-per-view.service.js'));

},{"../../common/1.3/dice/dice.module":309,"../../common/1.3/directive/float-label.directive":311,"../../common/1.3/filter/percent-filter/percent.filter":312,"./pay-per-view.config.js":292,"./pay-per-view.controller.js":293,"./pay-per-view.service.js":295,"./view-counts.controller.js":296}],295:[function(require,module,exports){
'use strict';

module.exports = function PayPerViewService($http, diceConstant, diceApi) {

    var ppv = {};

    ppv.get = function (id) {
        // if 404 dont show or call count
        return $http.get(diceConstant.proxyUrl + '/companies/' + id + '/ppv');
    };

    ppv.post = function (id, payload) {
        return $http.post(diceConstant.proxyUrl + '/companies/' + id + '/ppv', payload);
    };

    ppv.put = function (id, payload) {
        return $http.put(diceConstant.proxyUrl + '/companies/' + id + '/ppv', payload);
    };

    ppv.delete = function (id) {
        return $http.delete(diceConstant.proxyUrl + '/companies/' + id + '/ppv');
    };

    ppv.getCounts = function (id, payload) {
        return $http.get(diceConstant.proxyUrl + '/companies/' + id + '/ppv/counts', payload);
    };

    ppv.getCompanyName = function (id) {
        return $http.get(diceConstant.proxyUrl + '/companies/' + id + encodeURIComponent('?fields=name'));
    };

    ppv.remainingPercent = function (items) {
        var i, toPercent;
        for (i = 0; i < items.length; i++) {
            toPercent = 0;
            toPercent = items[i].totalTermViews / items[i].viewLimit;
            if (!toPercent) {
                toPercent = 0;
            }
            items[i].currentPercent = toPercent;
        }
    ...
    ppv.getPPV = function () {
        var items = diceApi.get('/companies/ppv/counts');
...
module.exports = function ViewCountsController($q, PayPerViewService, $filter) {
    ...
    vm.reversePercent = false;
    vm.reverseCompany = false;

    PayPerViewService.getPPV().then(function (response) {
        vm.viewCounts.items = response.data.items;
        vm.viewCounts.items = PayPerViewService.remainingPercent(vm.viewCounts.items);
    }, function (error) {
        console.log(error);
    });
};
...
module.exports = function diceContainedLoader(diceConstant, $rootScope, $timeout) {

    var directive = {
        restrict: 'E',
        template: '<div class="loader"><span>Loading...</span></div>',
        scope: {}
    };
...
module.exports = function diceFullscreenLoader(diceConstant, $rootScope, $timeout) {
...
module.exports = function diceLoader(diceConstant) {
    ...
    function templateFn(tElem, tAttrs) {
        if (tAttrs.diceLoader === 'contained') {
            return diceConstant.domains.customerAssetsProxy + '/src/common/1.3/component/loader/loader.contained.template.html';
        } else {
...
angular.module('dice.component.loader', []).run(run).directive('diceLoader', require('./loader.component.js')).directive('diceResolveLoader', require('./resolve-loader.component.js')).directive('diceContainedLoader', require('./contained-loader.component.js')).directive('diceFullscreenLoader', require('./fullscreen-loader.component.js'));

function run($rootScope, $templateCache) {
    $templateCache.put('loader.template.html', '<div class="loader"><span ng-transclude></span></div>');

    //$rootScope.$on('$routeChangeError', routeChangeError);

    // function routeChangeError(angularEvent, current, previous, rejection) {
    //     $rootScope.$broadcast('show.dice.toast', 'danger', rejection.data.message.messages[0].userMessage);
    // }
}

},{"./contained-loader.component.js":297,"./fullscreen-loader.component.js":298,"./loader.component.js":299,"./resolve-loader.component.js":301}],301:[function(require,module,exports){
'use strict';

module.exports = function diceResolveLoader(diceConstant, $rootScope, $timeout) {
...
module.exports = function diceModalShow($rootScope) {
    var directive = {
        restrict: 'A',
        scope: {
            diceModalShow: '@'
    ...
    function link(scope, element, attrs, controller) {
        element.bind('click', function (e) {
            e.preventDefault();
            this.blur();
            $rootScope.$broadcast('show.dice.modal', scope.diceModalShow, attrs.diceModalSize);
        });
    }
};
...
module.exports = function diceModal($timeout) {
    ...
    function link(scope, element, attrs, controller) {
        ...
        var diceModal = element.find('#dice-modal');

        scope.$on('show.dice.modal', function (e, template, size) {
            $timeout(function () {
                controller.diceTemplate = template.toString();
            }, 0);
            element.find('.modal-dialog').removeClass('modal-xxl modal-xl modal-lg modal-sm');
            if (size === 'xxl') {
                element.find('.modal-dialog').addClass('modal-xxl');
            }
            if (size === 'xl') {
                element.find('.modal-dialog').addClass('modal-xl');
            }
            if (size === 'lg') {
        ...
        scope.$on('hide.dice.modal', function (e) {
        ...
        diceModal.on('hidden.bs.modal', function () {
            scope.$apply(function () {
                controller.diceTemplate = '';
            });
        });
    }
};

},{"./modal.template.html":305}],304:[function(require,module,exports){
'use strict';

angular.module('dice.component.modal', []).directive('diceModal', require('./modal.component.js')).directive('diceModalShow', require('./modal-show.component.js'));

},{"./modal-show.component.js":302,"./modal.component.js":303}],305:[function(require,module,exports){
module.exports = "<div id=\"dice-modal\" class=\"dice-modal modal fade\">\n    <div class=\"modal-dialog\">\n        <div class=\"modal-content\">\n            <ng-include src=\"diceModalCtrl.diceTemplate\"></ng-include>\n        </div>\n    </div>\n</div>\n";

},{}],306:[function(require,module,exports){
'use strict';

angular.module('dice.component.toast', []).directive('diceToast', diceToast);

function diceToast($timeout) {

    var directive = {
        restrict: 'E',
        template: require('./toast.template.html'),
        scope: {},
        controller: controller,
        controllerAs: 'diceToastCtrl',
        bindToController: true,
        link: link
    };
    ...
    function controller($scope) {
        ...
        $scope.$on('show.dice.toast', function (e, type, message, internalMessage) {
            $scope.show(type, message, internalMessage);
        });
    }
    ...
    function link(scope, element, attrs, ctrl) {
        ...
        scope.add = function () {
            var alertHtml = '<div class="alert alert-dismissible dice-alert fade">' + '<button type="button" class="close" data-dismiss="alert" aria-label="Close"><i class="icon-cross"></i></button>' + '<span class="dice-alert-message"></span>' + '</div>';
        ...
        scope.show = function (type, message, internalMessage) {
            $timeout.cancel(timer);
            ...
            alert.find('.dice-alert-message').html(message);
            if (internalMessage) {
                alert.find('.dice-alert-message').append('<br/><small>' + internalMessage + '</small>');
            }
            alert.addClass('in');
...
},{"./toast.template.html":307}],307:[function(require,module,exports){
...
function Config($locationProvider, $sceDelegateProvider, $httpProvider, $compileProvider) {
...
function interceptor($q, $window, diceConstant) {
    return {
        responseError: function responseError(error) {
            switch (error.status) {
                case 401:
                    $window.location.assign(diceConstant.domains.customer + '/login');
                    break;
            }
...
require('../component/loader/loader.module.js');

require('../component/toast/toast.component.js');

require('../component/modal/modal.module.js');

var _diceConfig = require('./dice.config.js');

var _diceConfig2 = _interopRequireDefault(_diceConfig);

var _diceService = require('./dice.service.js');

var _diceService2 = _interopRequireDefault(_diceService);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

angular.module('dice', ['dice.component.loader', 'dice.component.toast', 'dice.component.modal']).value('dice', dice).constant('diceConstant', dice.constants).config(_diceConfig2.default).service('diceApi', _diceService2.default);

},{"../../../../node_modules/babel-polyfill":1,"../component/loader/loader.module.js":300,"../component/modal/modal.module.js":304,"../component/toast/toast.component.js":306,"./dice.config.js":308,"./dice.service.js":310}],310:[function(require,module,exports){
'use strict';
...
var diceApi = function () {
    function diceApi($http, diceConstant) {
        _classCallCheck(this, diceApi);

        this.$http = $http;
        this.diceConstant = diceConstant;
    }

    _createClass(diceApi, [{
        key: 'get',
        value: function get(path, timeout, config) {
            if (timeout) {
                return this.$http.get(this.diceConstant.proxyTimeoutUrl + '?timeout=' + timeout + '&path=' + encodeURIComponent(path), config);
            } else {
                return this.$http.get(this.diceConstant.proxyUrl + encodeURIComponent(path), config);
            }
        }
    }, {
        key: 'post',
        value: function post(path, data, timeout, config) {
            if (timeout) {
                return this.$http.post(this.diceConstant.proxyTimeoutUrl + '?timeout=' + timeout + '&path=' + encodeURIComponent(path), data, config);
            } else {
                return this.$http.post(this.diceConstant.proxyUrl + encodeURIComponent(path), data, config);
            }
        }
    }, {
        key: 'put',
        value: function put(path, data, timeout, config) {
            if (timeout) {
                return this.$http.put(this.diceConstant.proxyTimeoutUrl + '?timeout=' + timeout + '&path=' + encodeURIComponent(path), data, config);
            } else {
                return this.$http.put(this.diceConstant.proxyUrl + encodeURIComponent(path), data, config);
            }
        }
    }, {
        key: 'delete',
        value: function _delete(path, data, timeout, config) {
            if (timeout) {
                return this.$http.delete(this.diceConstant.proxyTimeoutUrl + '?timeout=' + timeout + '&path=' + encodeURIComponent(path), data, config);
            } else {
                return this.$http.delete(this.diceConstant.proxyUrl + encodeURIComponent(path), data, config);
            }
        }
    }]);
...
(function () {
    ...
    angular.module('dice.directive.floatLabel', []).directive('diceFloatLabel', diceFloatLabel);

    function diceFloatLabel() {
        ...
        return {
            ...
            link: function link(scope, element) {
                ...
                function labelOn() {
                    if (!element.hasClass('dice-float-label-on')) {
                        element.addClass('dice-float-label-on');
                    }
                }

                function labelDefault() {
                    if (input.val() === '' && element.hasClass('dice-float-label-on')) {
                        element.removeClass('dice-float-label-on');
                    }
                }
...
angular.module('dice.filter.percent', []).filter('dicePercentage', ['$filter', function ($filter) {
